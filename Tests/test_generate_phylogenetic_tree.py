from pathlib import Path
import os, sys, importlib
import pytest

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

mod = importlib.import_module("Main.generate_phylogenetic_tree")
ENTRY = getattr(mod, "main", None) or getattr(mod, "generate_phylogenetic_tree", None)


@pytest.mark.skipif(ENTRY is None, reason="generate_phylogenetic_tree entry function not found")
def test_generate_tree_writes_outputs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    data = tmp_path / "data"
    results = tmp_path / "Results"
    data.mkdir()
    results.mkdir()

    # Tiny alignment file if your script expects one
    aln = results / "alignment_input.fasta"
    aln.write_text(">A\nANACGT\n>B\nNACGAA\n>C\nTTTTTT\n", encoding="utf-8")

    monkeypatch.setenv("DOG_DATA_DIR", str(data))
    monkeypatch.setenv("DOG_RESULTS_DIR", str(results))

    # Support both call styles
    try:
        ENTRY()
    except TypeError:
        ENTRY(str(data), str(results))

    out = list(results.glob("*.png")) + list(results.glob("*.nwk")) + list(results.glob("*.tree*"))
    assert out, "No tree outputs written"
